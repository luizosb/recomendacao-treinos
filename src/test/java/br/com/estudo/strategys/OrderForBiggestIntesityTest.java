...
class OrderForBiggestIntesityTest {
    ...
    void testOrderByBigIntesity(){
        var listTraining = trainingSession.setStrategy(new OrderForBiggestIntesity());

        assertEquals("Agachamento", listTraining.get(0).getName());
        assertEquals("Supino", listTraining.get(1).getName());
        assertEquals("Remada", listTraining.get(2).getName());
